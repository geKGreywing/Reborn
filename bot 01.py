...
def pricecheck():
    global price1
    response = requests.get("https://tw.stock.yahoo.com/quote/2498")
    soup = BeautifulSoup(response.text, 'lxml')
    try:
        price1 = soup.find('span', {'class': 'Fz(32px) Fw(b) Lh(1) Mend(16px) D(f) Ai(c) C($c-trend-up)'}).getText() ##漲
        return float(price1)
    except:
        pass

    try:
        price1 = soup.find('span', {'class': 'Fz(32px) Fw(b) Lh(1) Mend(16px) D(f) Ai(c) C($c-trend-down)'}).getText() ##跌
        return float(price1)
    except:
        pass

    try:
        price1 = soup.find('span', {'class': 'Fz(32px) Fw(b) Lh(1) Mend(16px) D(f) Ai(c)'}).getText() ##平
        return float(price1)
    except:
        pass
...
while True:
    ...
    if status >=2 and status <4:
        if price2330>=first*(1.03) and status == 2:
            sell(5)
            status = 3
        if status==3:
            if price2330>=first*(1.06):
                sell(5)
                status = 4
            if price2330<=first:
                sell(5)
            ...
            if price2330<=first*(0.97):
                sell(10)
                status =6

    if status <=-2 and status >-4:
        if price2330<=first*(0.97) and status==-2:
            buy(5)
            status = -3
        if status==-3:
            if price2330<=first*(0.94):
                buy(5)
                status = -4
            if price2330==first:
                buy(5)
